...
#ifdef ENDSTOP_BEEP
void EndstopBeep() {
  static uint8_t last_status = ((READ(Z_MIN_PIN) << 3) | (READ(X_MIN_PIN) << 2) | (READ(Y_MIN_PIN) << 1) | READ(X_MAX_PIN));
  const uint8_t now_status = ((READ(Z_MIN_PIN) << 3) | (READ(X_MIN_PIN) << 2) | (READ(Y_MIN_PIN) << 1) | READ(X_MAX_PIN)) & 0xff;
