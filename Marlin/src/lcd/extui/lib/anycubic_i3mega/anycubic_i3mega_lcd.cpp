...
void AnycubicTFTClass::OnSetup() {
  ...
  SelectedFile[0] = 0;
    ...
    ExtUI::injectCommands_P(PSTR("M300 P200 S2794\nM300 P200 S2093\nM300 P250 S2349\nM300 P250 S2637\nM300 P500 S2349\nM300 P500 S3136\nM300 P750 S3520"));
  #endif
  #if ENABLED(ANYCUBIC_LCD_DEBUG)
        ...
        break;
          ...
          case '0':
            switch (SelectedDirectory[2]) {
               ...
               case '1': // "<01LoadFil>"
                SERIAL_ECHOLNPGM("Special Menu: Load Filament");
                ExtUI::injectCommands_P(PSTR("M701\nM300 P105 S1108\nM300 P210 S1661"));
                break;             
              
              case '2': // "<02UnLoadFil>"
                SERIAL_ECHOLNPGM("Special Menu: Unload Filament");
                // need to dwell for half a second to give the fan a chance to start before the pid tuning starts
              ...
              case '4': // "<04xxxx>" no used for moment
                break;                
              
              case '5': // "<05PrehtBed>"
                SERIAL_ECHOLNPGM("Special Menu: Preheat Bed");
                ExtUI::injectCommands_P(PSTR("M190 S80\nM300 P105 S1108\nM300 P210 S1661"));
                break;
              ...
              case '8': // "<08HtbedPID>"
                SERIAL_ECHOLNPGM("Special Menu: Auto Tune Hotbed Pid");
                ExtUI::injectCommands_P(PSTR("M303 E-1 S80 C10 U1\nM500"));
                break;
          ...
          case '1':
            switch (SelectedDirectory[2]) {
              ...
              case '1': // "<11LdEEPROM>"
                SERIAL_ECHOLNPGM("Special Menu: Load EEPROM");
                ExtUI::injectCommands_P(PSTR("M501\nM300 P105 S1108\nM300 P210 S1661"));
                break;
                
              case '2': // <12SendM81>
                SERIAL_ECHOLNPGM("Special Menu: Printer OFF");
                ExtUI::injectCommands_P(PSTR("M81"));
                break; 
...
void AnycubicTFTClass::RenderSpecialMenu(uint16_t selectedNumber) {
  switch (selectedNumber) {
      ...
      case 0: // 1ere Page
        SENDLINE_PGM("<01LoadFil>");
        SENDLINE_PGM("Load Filament");
        SENDLINE_PGM("<02UnLoadFil>");
        SENDLINE_PGM("Unload Filament");  
        SENDLINE_PGM("<12SendM81>");
        SENDLINE_PGM("Printer OFF");     
        SENDLINE_PGM("<Exit>");
        SENDLINE_PGM("Exit");
        break;
      ...
      case 4: // 2eme Page
        ...
        SENDLINE_PGM("<03SendM108>");
        SENDLINE_PGM("Send User Confirm."); 
        SENDLINE_PGM("<05PrehtBed>");
        SENDLINE_PGM("Preheat Bed");
        SENDLINE_PGM("<06ABL>");
        SENDLINE_PGM("Auto Bed Leveling");        
        SENDLINE_PGM("<07HtEndPID>");
        SENDLINE_PGM("Auto Tune Hotend PID");
        break;
        
      case 8: // 3eme Page
        SENDLINE_PGM("<08HtBedPID>");
        SENDLINE_PGM("Auto Tune Hotbed PID");      
        SENDLINE_PGM("<09FWDeflts>");
        SENDLINE_PGM("Load FW Defaults");
        SENDLINE_PGM("<10SvEEPROM>");
        SENDLINE_PGM("Save EEPROM");
        SENDLINE_PGM("<11LdEEPROM>");
        SENDLINE_PGM("Load EEPROM");
        break;
      ...
      if ((strchr(TFTcmdbuffer[TFTbufindw], 'A') != NULL)) {
        ...
        switch (a_command) {
          ...
          case 7: { // A7 GET PRINTING TIME
            const uint32_t elapsedMinutes = ExtUI::getProgress_seconds_elapsed() / 60;
            SEND_PGM("A7V ");
          ...
          case 16: { // A16 set hotend temp
            ...
            if (CodeSeen('S')) {
              tempvalue = constrain(CodeValue(), 0, HEATER_0_MAXTEMP);
